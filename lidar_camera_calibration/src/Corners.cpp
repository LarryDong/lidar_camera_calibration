...
bool getCorners(cv::Mat img, pcl::PointCloud<pcl::PointXYZ> scan, cv::Mat P, int num_of_markers, int MAX_ITERS){
	ROS_INFO("Begin to get corners......");

	// cv::Mat edge_mask = cv::Mat::zeros(img.size(), CV_8UC1);
	// edge_mask(cv::Rect(0, 0, img.cols, img.rows)) = 1;				// edge_mask = 1.
	...
	// pcl::PointCloud<pcl::PointXYZ> pc = scan;		//~ meanless. and removed.
	...
	for(pcl::PointCloud<pcl::PointXYZ>::iterator pt = scan.points.begin(); pt < scan.points.end(); pt++){
		// behind the camera
		if (pt->z < 0)
			continue;
	...
	std::vector<int> LINE_SEGMENTS(num_of_markers, 4); //assuming each has 4 edges and 4 corners. ~ '4' is the initialization value. LING_SEGMENT={4}
	...
	for (int q = 0; q < num_of_markers; q++){
		...
		for(int i=0; i<LINE_SEGMENTS[q]; i++){
			...
			// get markings in the first iteration only. ~ only label the marker for the first time.
			if(iteration_count == 0){
				polygon.clear();
				collected = 0;
				//~ NOTE: Everytime a key is pressed, the position is saved. So make sure to press after click.
				while(collected != LINE_SEGMENTS[q]){
					cv::setMouseCallback("cloud", onMouse, &_point_);
					cv::imshow("cloud", image_edge_laser);
					cv::waitKey(0);
					std::cout << "[Debug]. Clicked points " << collected << ", (" << _point_.x << ", " << _point_.y << ")." << endl;
					++collected;
			...
			polygon = stored_corners[4*q+i]; 		// TODO:
			...
			for(std::map<std::pair<int, int>, std::vector<float> >::iterator it=c2D_to_3D.begin(); it!=c2D_to_3D.end(); ++it){
				//~ cv::pointPolygonTest(contour, pt, true/false): check a point in polygon. If set 'true', return 1 if inside.
				if (cv::pointPolygonTest(cv::Mat(polygon), cv::Point(it->first.first, it->first.second), true) > 0){
					cloud->push_back(pcl::PointXYZ(it->second[0],it->second[1],it->second[2]));		//~ TODO: why inside?? (Check 2D-3D points in polygon?)
					rectangle(combined_rgb_laser, cv::Point(it->first.first, it->first.second), cv::Point(it->first.first, it->first.second), cv::Scalar(0, 0, 255), 3, 8, 0); //~ draw red points
				}
			}
			
			ROS_WARN_STREAM("Pts number inside the polygon: " << cloud->points.size());
			if(cloud->size() < 2){ 
				ROS_WARN("Not enough corners in this marker...");
				return false;
			}
		...
		for(int i=0; i<LINE_SEGMENTS[q]; i++){
			pcl::lineToLineSegment(line_model[i], line_model[(i+1)%LINE_SEGMENTS[q]], p1, p2);	//~ Get the shortest 3D segment between two 3D lines
			for(int j=0; j<4; j++){
		...
		*board_corners += *corners;
		...
		for(int i=0; i<4; i++){
			std::cout << sqrt(pow(c_3D[4*q+i].x - c_3D[4*q+(i+1)%4].x, 2)
							+ pow(c_3D[4*q+i].y - c_3D[4*q+(i+1)%4].y, 2)
							+ pow(c_3D[4*q+i].z - c_3D[4*q+(i+1)%4].z, 2)) << std::endl;
		}
	}
	outfile.close();
